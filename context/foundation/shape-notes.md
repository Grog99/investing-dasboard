...
checkpoint:
  ...
  gray_areas_resolved:
    - topic: "differentiator / insight"
      decision: "AI research + notes as a first-class part, tied to each company; plus one unified view combining portfolio + watchlist + news + research"
    - topic: "primary persona scope"
      decision: "single user (self) for now; keep a door open to share with others later"
    - topic: "view shape"
      decision: "both — an overview dashboard for the glance, plus per-company drill-down"
    - topic: "authentication / entry"
      decision: "full login with accounts (email+password or OAuth) — prepares for sharing later"
    - topic: "role model"
      decision: "flat — every user sees only their own data; full per-account data isolation; no admin/roles"
    - topic: "MVP scope depth"
      decision: "keep all three pillars (portfolio, monitoring, AI research) but thinnest viable version of each: manual portfolio entry (no XTB auto-sync), one API data source (no custom scrapers), one-shot AI research saved as a note (no agentic pipeline). Scrapers and XTB sync deferred to v2."
    - topic: "MVP timeline"
      decision: "~3 weeks after-hours (tight target). User chose scope-down path to keep it tight."
    - topic: "product framing"
      decision: "web-app; small scale (self / a few people); no hard deadline; after-hours only"
    - topic: "non-goals"
      decision: "no buy/sell recommendations or trade signals; no XTB auto-sync in MVP; no realtime/tick data; no tax/accounting/reporting"
  frs_drafted: 13
  quality_check_status: accepted
---

# Shape Notes — Investing Dashboard

> Seed idea (verbatim, PL): Chciałem stworzyć mój prywatny dashboard do inwestycji.
> Chciałbym połączyć w nim informacje o tym ile jakich akcji mam, miejsce w którym
> będę mógł śledzić moje spółki, informacje ile zarobiły. Ale też informacje o
> spółkach których nie mam ale chcę obserwować. Dla obu typów spółek chciałbym też
> obserwować informacje czy to przez scrapowanie stron czy w inny sposób. I ostatnią
> rzeczą jaką będzie ten dashboard to będzie miejsce w którym będę mógł robić własne
> researche na temat spółek, robić sobie notatki, zapisywać informacje, wykresy itd.
> Do researchy chciałbym używać modeli AI, ale z pozostawieniem opcji dodawania
> danych samemu.
...
Captured decision — view shape: the product has **both** an overview dashboard (the
glance: portfolio value, gains, signals and news across all companies) **and** a
per-company drill-down page (one company in full: position, news, own research,
charts). Overview → drill-down navigation.
...
## MVP scope note (thinned pillars)

All three pillars are present in the MVP but at their thinnest viable depth. Deferred
to v2 (captured so they are not lost):

- Automatic portfolio sync with XTB (MVP: manual entry).
- Custom per-source web scrapers for news/data (MVP: one API data source).
- Full agentic AI research pipeline (MVP: one-shot research saved as a note).
- Charts / image attachments in research notes (MVP: text notes only).
...
Run on 2026-07-06. Result: `accepted` — all elements present, no gaps.

- Access Control — present (accounts, flat model, per-account isolation).
- Business Logic — present (one-sentence synthesis rule; not empty CRUD).
- Project artifacts — present (this file with a valid checkpoint block).
- Timeline-cost — present (MVP target 3 weeks; scope deliberately thinned).
- Non-Goals — present (four explicit non-goals, including the no-recommendations boundary).
